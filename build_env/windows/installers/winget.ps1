...
function Install-WingetTool
{
    ...
    if ((Test-Path $Marker) -and ($installed -match [regex]::Escape($Version))) {
        Add-ToMachinePath -Marker $Marker
        return
    }
    winget install --exact --id $Id --version $Version --location $Location --scope machine --silent --accept-package-agreements --accept-source-agreements --disable-interactivity
...
{
    param($Id, $Marker)
    winget uninstall --exact --id $Id --scope machine --silent --accept-source-agreements 2>$null
